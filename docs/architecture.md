...
| **Embedding Model** | `intfloat/multilingual-e5-small` vs larger models | Chose `e5-small` (384-dim) over larger models like `e5-large` (1024-dim) or `bge-m3` for three reasons: (1) fits in 1 GB RAM alongside the cross-encoder, (2) inference under 100ms on CPU, (3) supports 100+ languages including Portuguese. Larger models would improve accuracy but require more RAM and slower inference — a trade-off not justified for a PoC. In production with adequate hardware, a larger model would be a straightforward upgrade. |
| **Agentic RAG** | Function calling for multi-step reasoning | Not used. Agentic RAG would add LLM calls per question (decision loop, tool selection), increasing both cost and latency. For factual Q&A about food and wine, a fixed pipeline (rewrite → search → rerank → generate) is sufficient and more predictable. Cost-conscious design for a PoC — agentic behavior can be added later if use cases require complex multi-step queries. |
